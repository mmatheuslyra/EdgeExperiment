...
def get_container_network_usage(container_id):
    try:
        ...
        total_rx = sum(network['rx_bytes'] for network in network_stats.values())
        total_tx = sum(network['tx_bytes'] for network in network_stats.values())
        network_percentage = round((total_rx + total_tx) / 2, 2)
...
while time.time() - start_time <= monitoring_duration:
    ...
    network_percentage_broker = get_container_network_usage(container_id_broker)
    ...
    network_percentage_iot = get_container_network_usage(container_id_iot)
    ...
    network_percentage_subscriber = get_container_network_usage(container_id_subscriber)
    ...
    if (
        ...
        # Update the network usage chart for each container
        ax3.clear()
        ax3.plot(time_data, network_data_broker, label='Edge Broker', color='tab:blue')
        ax3.plot(time_data, network_data_iot, label='IoT Dev Sim', linestyle='dashed', color='tab:orange')
        ax3.plot(time_data, network_data_subscriber, label='Edge Subscriber', linestyle='dotted', color='tab:green')
