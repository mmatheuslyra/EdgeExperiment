      ...
      publishTopic.forEach(topic=>{
        ...
        axios.post('http://cloudServer:3020') // Send message to Cloud server    ajustar endpoit da cloud
          .then(response => {
            ...
            //FIXME: Ajustar pra que os valores dos graficos fiquem ok



            // axios.get('http://mongodb:3001/devices/:'+device+'/:'+topic,(req,res,next)=>{  //FIXME: Isso aqui tem que ser pego no banco
            axios.get('http://mongodb:27017/devices/:'+device+'/:'+topic,(req,res,next)=>{  //FIXME: Isso aqui tem que ser pego no banco
                DeviceModel.aggregate([{$match:{'deviceId':req.params.deviceId, 
                                        'deviceParameter':req.params.deviceParameter}},
                                      {$group: {_id:null, average: {$avg: '$deviceValue'}}}
                ]).exec().then(result=>{;
